...
FEATURE_NAMES = ["recent_discharge","abnormal_lab_count","claims_last_60d","med_refill_count"]
...
def extract_features(events: List[Dict[str, Any]]):
    ...
    Returns:
        List[int]: List of feature values.
    """
    ...
    return [recent_discharge, abn_count, claims_60, refill_count]
...
def train_model(seed: int = 42):
    ...
    X, y = [], []
    # Generate synthetic training set with rule-derived labels
    for _ in range(800):
        ev = gen_events(14)
        f = extract_features(ev)
        # Simple rule to derive risk class for demo purposes
        score = f[0]*3 + (f[1]//2)*2 + (f[2]//2)*2 + (0 if f[3]>0 else 1)
        label = 2 if score > 4 else (1 if score > 2 else 0)  # 0=LOW,1=MEDIUM,2=HIGH
...
def get_snapshot(member_id: str):
    ...
    fvec = np.array(extract_features(events)).reshape(1,-1)
    ...
    try:
        ...
        # For multiclass, TreeExplainer returns a list (one per class); otherwise an array
        shap_class = shap_values[pred_class] if isinstance(shap_values, list) else shap_values
        # Force to a 1‑D float vector regardless of shape/version
        sv = np.ravel(np.asarray(shap_class)).astype(float)
        # Pick top contributing features by absolute impact
    ...
    return {
        ...
        "risk": {"score": float(max(probs)), "label": label, "factors": factors},
        "summary": summary
    }
